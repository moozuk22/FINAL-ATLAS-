...
async function migrateTable(tableName: string): Promise<MigrationResult> {
  ...
  try {
    ...
    // Използваме batch insert за по-големи таблици
    const batchSize = 100;
    let imported = 0;
    ...
    for (let i = 0; i < oldData.length; i += batchSize) {
      ...
      const cleanedBatch = batch.map((row: any) => {
        ...
        // Премахни полета които може да не са дефинирани в новата схема
        delete cleaned.completed_at; // Това поле може да не съществува в някои таблици
        return cleaned;
      });
      ...
      if (batchError) {
        ...
        console.log(`⚠️  Upsert failed for batch ${i}-${i + batchSize}, trying insert...`);
...
async function migrateAll() {
  console.log('🚀 Starting Supabase migration using MCP data...');
